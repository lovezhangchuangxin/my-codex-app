...
export class AppServerClient extends EventEmitter {
  ...
  async initialize(): Promise<InitializeResult> {
    ...
    const response = await this.#transport.sendRequest<InitializeParams, InitializeResult>(
      "initialize",
      {
        clientInfo: {
  ...
  async listThreads(params: ThreadListParams): Promise<ThreadListResult> {
    ...
    return this.#transport.sendRequest<ThreadListParams, ThreadListResult>("thread/list", params);
  }
  ...
  async resumeThread(threadId: string): Promise<ThreadResumeResult> {
    this.#assertInitialized();
    return this.#transport.sendRequest<ThreadResumeParams, ThreadResumeResult>("thread/resume", {
      threadId
    });
  }
  ...
  async startTurn(params: TurnStartParams): Promise<TurnStartResult> {
    this.#assertInitialized();
    return this.#transport.sendRequest<TurnStartParams, TurnStartResult>("turn/start", params);
  }

  async listModels(params: ModelListParams): Promise<ModelListResult> {
    this.#assertInitialized();
    return this.#transport.sendRequest<ModelListParams, ModelListResult>("model/list", params);
  }

  async interruptTurn(params: TurnInterruptParams): Promise<void> {
    this.#assertInitialized();
    await this.#transport.sendRequest<TurnInterruptParams, unknown>("turn/interrupt", params);
  }

  async readFile(path: string): Promise<FsReadFileResult> {
    this.#assertInitialized();
    return this.#transport.sendRequest<FsReadFileParams, FsReadFileResult>("fs/readFile", {
      path
    });
  }
  ...
  async readDirectory(path: string): Promise<FsReadDirectoryResult> {
    this.#assertInitialized();
    return this.#transport.sendRequest<FsReadDirectoryParams, FsReadDirectoryResult>(
      "fs/readDirectory",
      {
        path
      }
  ...
  async #readThread(threadId: string, includeTurns: boolean): Promise<ThreadReadResult> {
    this.#assertInitialized();
    return this.#transport.sendRequest<ThreadReadParams, ThreadReadResult>("thread/read", {
      threadId,
      includeTurns
    });
  }

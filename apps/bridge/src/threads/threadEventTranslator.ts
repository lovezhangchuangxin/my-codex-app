...
export class ThreadEventTranslator {
  ...
  toNotificationBridgeEvent(method: string, params: unknown): BridgeEvent | null {
    ...
    switch (method) {
      case "thread/started": {
        ...
        const thread = attachThreadRuntime(
          this.cache,
          toThreadDetail(rawThread, this.cache.listPendingRequests(rawThread.id))
        );
      ...
      case "thread/tokenUsage/updated": {
        const threadId = asString(payload.threadId);
        const tokenUsage = toObject(payload.tokenUsage) as AppServerThreadTokenUsage | null;
